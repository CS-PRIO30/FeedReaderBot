...
def init_DB():
	...
	cursor = conn.cursor()
	cursor.execute("""CREATE TABLE IF NOT EXISTS urlSites (url text primary key, int chat_id) """)           
	cursor.execute("""CREATE TABLE IF NOT EXISTS feedUser (url text primary key, base_url text, chat_id int , name text, dateAdded text, active int) """)
				...
				if update.message:
					...
					try:	
						if "/list" in update.message.text:
							...
							conn = sqlite3.connect( DATABASE_NAME ) 
							cursor = conn.cursor()
							cursor.execute("""SELECT url FROM feedUser WHERE chat_id = {}""".format(chat_id) )
							conn.commit()
					...
					try:
						...
						print("error in removing feeds ", e)
					...
					try:
						if update.message.entities[0].type == "url":
							if chat_id in waitListAdd:
								...
								try:
									now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") 
									cursor.execute( "INSERT OR IGNORE INTO feedUser VALUES (?,?,?,?,?,?)", (url,"",chat_id,username,now,1) )
									conn.commit()
